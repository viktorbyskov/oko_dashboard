...
with col2:
    udgifter = df[(df['Ind_ud'] == 'ud') & (df['Kategori'] != 'løn') & (df['Kategori'] != 'status')]['beløb'].sum()
    # Format the value using f-string and swap comma and period
...
with col3:
    # Måneder hvor der ikke er overført til budgetkonto
    months_budgetkonto = ["Januar"]
    # Get the specific value from the dataframe
    budgetkonto = df_ulon_ustatus["beløb_kvartal"].sum() / 3 + df_ulon_ustatus["beløb_år"].sum() / 12
    for i in sheet_name:
        if sheet_name in months_budgetkonto:
            budgetkonto = 0.0
    # Format the value using f-string and swap comma and period
